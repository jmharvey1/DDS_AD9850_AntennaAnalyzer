...
int OptionCnt = 12; //Total test option count
...
double current_freq = 0.0; // used in the runcurve routine to track frequency
double VSWRLim = 1.7;
int FwdOffSet;
int RevOffSet;
...
//  else  Serial.print ("- ");
   if(StartSwp && virtualPosition == OptionCnt){
    
     if(IncDecVal == +1) current_freq = current_freq +1000.0;
     if(IncDecVal == -1) current_freq = current_freq -1000.0;
...
void sendFrequency(double frequency) {
  frequency = frequency*(10.0000e6)/10e6;
...
bool PrintNextPoint(bool RunCurve){
  ...
  double REV;
  double VSWR;
  //double current_freq; moved to init area to make it a global value
  double EffOhms;
  bool GudRun = false;
  bool Incrmnt = true;
  LastVSWR = 999;
...
// Prepare Array for new Run
  ...
  while (RunCurve){
    ...
    if (VSWR < SweepData[1]) {  // SweepData[1] = BestSWR
      if (Incrmnt){
        SweepData[1] = VSWR;
...
void Splash_Screen(int option){
   ...
   switch (option) {
     case 1: //Set Freq Limits for 160_MTRS 
      Start_Freq = 1.7e6; // Freq; in Mhz
      End_Freq =  2.3e6; // Freq; in Mhz
      Step_Freq = 10e3; //Step  Freq; in KHz
      sprintf (str_Band, "160"); //str_Band = "40"
      break;

     case 2: //Set Freq Limits for 80_MTRS 
    ...
    case 3: //Set Freq Limits for 40_MTRS 
      ...
      break;
    case 4: //Set Freq Limits for 30_MTRS 
//      Start_Freq = 9.8e6; // Freq; in Mhz  20.5e6;//  3.6e6;//
//      End_Freq =  10.20e6; // Freq; in Mhz
      Start_Freq = 9.7e6; // Freq; in Mhz  20.5e6;//  3.6e6;//
      End_Freq =  10.50e6; // Freq; in Mhz

      Step_Freq = 01e3; //Step  Freq; in KHz
      sprintf (str_Band, "30");
      break;
    ...
    case 7: //Set Freq Limits for 10_MTRS 
      ...
      case 8: //Set Freq Limits for Special Fixed Freq 
      Start_Freq = 3.560e6; // Freq; in Mhz 
      End_Freq =  4.000e6; // Freq; in Mhz
      Step_Freq = 0.00e2; //Step  Freq; in 10Hz
      sprintf (str_Band, "T1");
      break;

      case 9: //Set Freq Limits for Special Fixed Freq
      Start_Freq = 7.0401e6; // Freq; in Mhz 
      End_Freq =  7.300e6; // Freq; in Mhz 
      //Start_Freq = 1.90e6; // Freq; in Mhz 
      //End_Freq =  2.000e6; // Freq; in Mhz
      Step_Freq = 0.00e2; //Step  Freq; in 10Hz
      sprintf (str_Band, "WSPR40");
      break;       
       

      case 10: //Set Freq Limits for Special Fixed Freq
      //Start_Freq = 7.13e6; // Freq; in Mhz 
      //End_Freq =  7.300e6; // Freq; in Mhz 
      Start_Freq = 3.5941e6; // Freq; in Mhz 
      End_Freq =  4.000e6; // Freq; in Mhz
      Step_Freq = 0.00e2; //Step  Freq; in 10Hz
      sprintf (str_Band, "WSPR80");
      break; 

      case 11: //Set Freq Limits for Special Fixed Freq
      Start_Freq = 10.1402e6; // Freq; in Mhz 
      End_Freq =  10.1403e6; // Freq; in Mhz
      Step_Freq = 0.00e2; //Step  Freq; in 10Hz
      sprintf (str_Band, "WSPR30");
      break; 

      case 12: //Set Freq Limits for Special Fixed Freq
 
      Start_Freq = 10.090e6; /// Freq; in Mhz 
//      Start_Freq = 9.680e6; /// Freq; in Mhz
      End_Freq =  10.160e6; // Freq; in Mhz
      Step_Freq = 0.00e3; //Step  Freq; in 10Hz
      sprintf (str_Band, "MANSCN");
      break; 

//      case 12: //Set Freq Limits for Special Fixed Freq
//      Start_Freq = 1.0e6;10.1403e6; // Freq; in Mhz 
//      End_Freq =  10.150e6; // Freq; in Mhz
//      Step_Freq = 40.00e3; //Step  Freq; in 10Hz
//      sprintf (str_Band, "T5");
//      break;       
//    default: 
